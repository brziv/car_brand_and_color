...
os.makedirs(f"{prefix}results/", exist_ok=True)
...
for model_name in model_names:
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            no_improve = 0
            # save best model
            torch.save(model.state_dict(), f"{prefix}results/{model_name}_best.pth")
        else:
            no_improve += 1
    ...
    with open(f"{prefix}results/{model_name}_test_metrics.json", "w") as f:
    ...
    plt.tight_layout()
    plt.savefig(f"{prefix}results/{model_name}_metrics.png")
    plt.close()
    ...
    plt.ylabel("Actual")
    plt.savefig(f"{prefix}results/{model_name}_cmatrix.png")
    plt.close()
